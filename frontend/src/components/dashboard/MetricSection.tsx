...
export function MetricSection({ title, metric, chartLabel, colorMode = "categorical" }: any) {
  ...
  const handleDownload = () => {
    if (data) exportToExcel(data, `${title.replace(/\s+/g, '_')}_Report`);
  }
    ...
    .map(key => ({ 
      ...
      cell: ({ row }: any) => {
        ...
        if (key === 'Period') return (
          <div className="p-3 flex flex-col leading-tight"><span className="font-bold text-slate-700 dark:text-slate-200">{val as string}</span><span className="text-[9px] text-slate-400 font-medium mt-0.5 tabular-nums">{range}</span></div>
        );
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 py-10 border-b border-slate-100 dark:border-slate-800 last:border-0">
      ...
      <div className="screener-table bg-white dark:bg-slate-900/40 rounded-xl border p-2 shadow-sm overflow-x-auto [&_td]:p-0 [&_td]:overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
            ...
            <div className="flex items-center gap-2 overflow-x-auto hide-scrollbar max-w-full">
              <div className="segmented-tabs-list !mb-0 shrink-0">
                {data.map((row: any) => (
                  <button key={row.Period} onClick={() => handlePeriodChange(row.Period)} className={`segmented-tab-trigger ${activePeriod === row.Period ? '!bg-white dark:!bg-slate-800 !text-blue-600 dark:!text-blue-400 shadow-sm' : ''}`}>{row.Period}</button>
                ))}
              </div>
            </div>
          </div>
